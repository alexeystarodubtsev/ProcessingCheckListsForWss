...
{
    class ObjectionsProcess : Program
    {
        public static XLWorkbook GetXLWorkbook(List<Manager> lm, DateTime firstDate, DateTime LastDate, XLWorkbook oldFile, string company = "default")
        {
            ...
            // нечего сложного этот скрипт ищет менеджеров и теги, далее их распологает на табличку, ориетиром случит ячейка с значением "статистика".
            List<string> nameManager = new List<string>();
            List<string> tagMOD = new List<string>();
            ...
            foreach (var page in wb.Worksheets)
            {
                ...
                for (int i = 1; i < rg.LastColumn().ColumnNumber(); i++)
                {
                    ...
                    {
                        if (tagMOD.Count() == 0) tagMOD.Add(page.Cell(3, i).GetString());

                        for (int j = 3; j < rg.LastRow().RowNumber(); j++)
                        {
                            for (int k = 0; k < tagMOD.Count(); k++)
                            {
                                if (tagMOD[k] == page.Cell(j, i).GetString()) break;
                                else if (k == tagMOD.Count() - 1) tagMOD.Add(page.Cell(j, i).GetString());
                            }
                        }
                    }
                }

                // Этот код записывает формулы в табичку, адреса ячеек опреляются от положения ячейки со значением "менеджер"
                for (int i = 1; i < rg.LastColumn().ColumnNumber(); i++)
                {
                    idxstart = i;
                    ...
                    if (page.Cell(1, i).GetString() == "Статистика")
                    {
                        ...
                        for (int j = 0; j < tagMOD.Count(); j++)
                        {
                            ...
                            page.Cell(2, j + idxstart + 1).SetValue<string>(tagMOD[j]);
                        }
                        ...
                        for (int j = 0, q = 0; j < tagMOD.Count(); j++)
                        {
                ...
                if (TF == false)
                {
                    string buffer = (string.Format("{0}",
                                    page.Cell(1, idxstart + tagMOD.Count() - 4).WorksheetColumn().ColumnLetter()));
